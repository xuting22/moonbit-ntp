...
function _M0MP211localreview3ntp6Packet6encode(self) {
  ...
  const _bind = [self.reference, self.origin, self.receive, self.transmit];
...
function _M0FP411localreview3ntp3cmd3web3run(input) {
  let _try_err;
  _L: {
    const _bind = _M0FP411localreview3ntp3cmd3web5unhex(input);
    let _tmp;
    if (_bind.$tag === 1) {
      const _ok = _bind;
      _tmp = _ok._0;
    ...
    const _bind$3 = _M0MP211localreview3ntp6Packet6encode(p);
    let _tmp$3;
    if (_bind$3.$tag === 1) {
      const _ok = _bind$3;
      _tmp$3 = _ok._0;
